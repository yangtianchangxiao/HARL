...
class SearchGrid(gym.Env):
    def __init__(self, map_set, map_num):
        ...
        # self.view_range = 10
        self.view_range = 5
        local_map_radius = 2*self.view_range # 10
        self.local_map_range = 2 * local_map_radius - 1 # 19
        self.observation_space = spaces.Box(low=0, high=1, shape=(1 * 60 * 60 + self.local_map_range**2,)) # 60*60是全局地图的大小，9*9是局部地图的大小
        self.share_observation_space = spaces.Box(low=0, high=1, shape=(1 * 60 * 60 + self.local_map_range**2,))
        # When use cnn
        # self.observation_space = spaces.Box(low=0, high=1, shape=(4, 50, 50,))
    ...
    def raise_difficulty(self):
        self.run_time = self.run_time + 10
    ...
    def state_action_reward_done(self, rescue_masks):  # 这里返回状态值，奖励值，以及游戏是否结束
        ...
        for drone_count, each_map in enumerate(single_map_set):
            ...
            # 计算 local view map 的大小
            local_view_height =  self.drone_list[drone_count].local_view_map.shape[1]
            local_view_width =  self.drone_list[drone_count].local_view_map.shape[2]

            # 创建一个全是 1 的数组，大小满足你的要求
            full_ones_array = np.ones((1, self.local_map_range, self.local_map_range)) # 这里是你希望得到的图像的大小，你需要用实际值替代这些

            # 将 local view map 复制到全是 1 的数组的左下角
            full_ones_array[:, :local_view_height, :local_view_width] = self.drone_list[drone_count].local_view_map
